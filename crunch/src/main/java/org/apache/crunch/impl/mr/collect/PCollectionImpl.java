...
public abstract class PCollectionImpl<S> implements PCollection<S> {
	...
	public long getSizeByRecord() {
		if (materializedAt != null) {
			long sz = materializedAt.getSizeByRecord(getPipeline().getConfiguration());
			if (sz > 0) {
				return sz;
			}
		}
		return getSizeByRecordInternal();
	}

	protected abstract long getSizeInternal();

	protected abstract long getSizeByRecordInternal();

	/**
	 * Retrieve the PCollectionImpl to be used for chaining within
	 * PCollectionImpls further down the pipeline.
	 * 
