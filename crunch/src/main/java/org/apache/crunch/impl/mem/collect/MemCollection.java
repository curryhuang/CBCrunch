...
public class MemCollection<S> implements PCollection<S> {
	...
	/**
	 * The method creates a {@link TaskInputOutputContext} that will just
	 * provide {@linkplain Configuration}. The method has been implemented with
	 * javaassist as there are API changes in versions of Hadoop. In hadoop
	 * 1.0.3 the {@linkplain TaskInputOutputContext} is abstract class while in
	 * version 2 the same is an interface.
	 * <p>
	 * Note: The intention of this is to provide the bare essentials that are
	 * required to make the {@linkplain MemPipeline} work. It lacks even the
	 * basic things that can proved some support for unit testing pipeline.
	 */
