...
public abstract class FileSourceImpl<T> implements Source<T> {
  ...
  public long getSizeByRecord(Configuration configuration){
	  try {
	      return SourceTargetHelper.getPathTextFileSizeByRecord(configuration, path);
	    } catch (IOException e) {
	      LOG.warn(String.format("Exception thrown looking up size by record of: %s", path), e);
	      throw new IllegalStateException("Failed to get the file size by record of:" + path, e);
	    }
  }
